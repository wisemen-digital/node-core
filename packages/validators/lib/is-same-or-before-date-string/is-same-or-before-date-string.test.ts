...
import { IsSameOrBeforeDateString } from './is-same-or-before-date-string.validator.js'

class TestClass {
  @IsSameOrBeforeDateString((obj: TestClass) => obj.reference)
  dateString: string

  reference: string
}

describe('IsSameOrBeforeTimeString decorator test', () => {
  it('should pass validation when the date string is before the reference', async () => {
  ...
  it('should fail validation when the date string is after the reference', async () => {
    const testInstance = new TestClass()
