...
describe('Csv Schema', () => {
  ...
  it('Parse a valid input', async () => {
    ...
    expect(parsed[0].name).toBe('Sijmkens')
    expect(parsed[0].firstName).toBe('Maarten')
    expect(parsed[0].birthdate).toBe('1997-04-09')
    expect(parsed[0].age).toBe(27)
    expect(parsed[0].gender).toBe(Gender.MALE)
    expect(parsed[0].agreed).toBe(true)
    expect(parsed[0].pets).toEqual([Pet.CAT, Pet.DOG])
    expect(parsed[0].remarks).toBe(null)
  })
})
